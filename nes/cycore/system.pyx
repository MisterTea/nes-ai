...
from .ppu cimport NESPPU, SCREEN_HEIGHT_PX, SCREEN_WIDTH_PX
...
from nes.ai.vlm import GptVisionLanguageModel
...
cdef class InterruptListener:
    ...
    def save(self):
        return (self._nmi, self._irq, self.dma_pause, self.dma_pause_count)

    def load(self, state):
        (self._nmi, self._irq, self.dma_pause, self.dma_pause_count) = state
...
cdef class NES:
    ...
    def save(self):
        return (self.cart.save(), self.cpu.save(), self.memory.save(), self.ppu.save(), self.apu.save(), self.controller1.save(), self.controller2.save(), self.interrupt_listener.save())
    ...
    def load(self, state):
        ...
        self.apu.load(apu)
        self.ppu.load(ppu)
        self.controller1.load(controller1)
        self.controller2.load(controller2)
