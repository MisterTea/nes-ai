...
def train_gmm(n_dim, n_components, learning_rate, epochs):
    ...
    model = DeepParallelGMM(
        INPUT_DIMENSIONS, INPUT_DIMENSIONS // 2, n_dim, n_components
    )
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)

    TRAIN_SAMPLES = 1000
    input_data = torch.ones(
        (TRAIN_SAMPLES, INPUT_DIMENSIONS), dtype=torch.float, device="mps"
    )
    sample_input_data = torch.ones(
        (TRAIN_SAMPLES * 10, INPUT_DIMENSIONS), dtype=torch.float, device="mps"
    )
    ...
    for epoch in range(epochs):
        with torch.no_grad():
            ...
            c = torch.distributions.Categorical(
                logits=torch.ones((2,), dtype=torch.float)
            )
            encoded_c = torch.nn.functional.one_hot(
                c.sample((TRAIN_SAMPLES * n_dim,)), num_classes=2
            )
        ...
        model_loss = 1.0 * (
            -torch.mean(outputs)
        )  # + 0.1 * (torch.norm(model.weights, 2) ** 2)# + (0.01 * torch.nn.MSELoss()(model.weights.sum(),torch.ones(1))) + (10 * torch.nn.L1Loss(reduction="sum")(model.weights,torch.clamp(model.weights.detach(), min=1e-2)))
