...
class ParallelGMM(torch.nn.Module):
    def __init__(self, n_dim, n_components, device="mps"):
        ...
        self.weights = torch.nn.Parameter(
            torch.ones(n_dim, n_components) / n_components
        ).to(device)
    ...
    def forward(self, x):
        ...
        log_probs2 = (
            -((x - self.means.reshape(1, self.n_dim, self.n_components)) ** 2)
            / (2 * var)
            - log_scale
            - math.log(math.sqrt(2 * math.pi))
        )
        # log_probs2 = log_probs2.unsqueeze(1)
        log_probs2 += torch.nan_to_num(torch.log(self.weights), nan=0.0).reshape(
            1, self.n_dim, self.n_components
        )
    ...
    def sample(self):
        ...
        encoded_c = torch.nn.functional.one_hot(
            c.sample((10000,)), num_classes=self.n_components
        ).to(self.means.device)
        # print(encoded_c.shape)
        # print(encoded_c)
        draws = (
            torch.randn(
                (10000, self.n_dim, self.n_components), device=self.means.device
            )
            * self.stds.expand(10000, self.n_dim, self.n_components)
        ) + self.means.expand(10000, self.n_dim, self.n_components)
        ...
        assert retval.shape == (
            10000,
            self.n_dim,
        ), f"{retval.shape} != (10000,{self.n_dim})"
        for x in range(self.n_dim):
            c = torch.distributions.Categorical(
                logits=torch.clamp(self.weights[x], min=1e-1)
            )
            encoded_c = torch.nn.functional.one_hot(
                c.sample((10000,)), num_classes=self.n_components
            )
        ...
        assert (
            retval.shape == retval_fast.shape
        ), f"{retval.shape} != {retval_fast.shape}"
